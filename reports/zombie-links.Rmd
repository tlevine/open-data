...
If we ignore the datasets that are stored internally and thus make the bars represent only the
datasets that are external links, the plot looks like this.

```{r p_link_types_onlylinks}
p.link.types.onlylinks
```

The missing bars correspond to catalogs with no external links.

...
It goes without saying that websites shouldn't have dead links, but here are
a bunch of fancy reasons why dead links are bad on open data sites.
...
* The Open Data Institute
    ...
    asks for a bunch of URLs, and I presume they should be alive. It also
    asks "How long will this data be available for?"
* The [questions](http://census.okfn.org/faq/#what-do-all-the-questions-about-the-datasets-mean-)
  ...
  * "URL of data online?"
* Under the [five-star linked open data scheme](http://www.w3.org/DesignIssues/LinkedData.html)
    "you get one (big!) star if the information has been made public at all,
    even if it is a photo of a scan of a fax of a table". I guess you still get
    this star if your link dies, as long as another
...
I got [a list of Socrata catalogs]()
([mirror]())
and [a list of CKAN catalogs]()
([mirror]()),
and I tried accessing the APIs for all of these sites.
...
From the metadata for each dataset, I
[determined](https://github.com/tlevine/open-data/blob/master/links.py)
whether the dataset was internally stored or externally stored.
...
```{r has_links_socrata, fig.width = 5, fig.height = 2}
p.has_links.socrata
```
...
have externally stored dataset. That's a big difference.

```{r has_links, fig.width = 5, fig.height = 2}
p.has_links
```
...
```{r software_all_types, fig.width = 8, fig.height = 6}
...
```{r prop_links_socrata, fig.width = 5, fig.height = 5}
...
```{r prop_links, fig.width = 5, fig.height = 5}
p.prop_links
```
...
```{r software_only_links, fig.width = 8, fig.height = 6}

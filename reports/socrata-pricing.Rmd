...
[pricing of Socrata](<%= pricing %>) they are.
...
The extended plan (Soc-OD-Ex) is basically this.

* 500 datasets
* 25 premium APIs
* Costs just under $10,000 per month

And the enterprise plan (Soc-OD-Ent) is basically this.
...
```{r fancy, echo = FALSE, fig.height = 8, fig.width = 8}
p.fancy
```
{:.wide}
...
kable(catalogs[c('data.portal.plan','data.collect.plan','has.311')])
```
...
```{r forms, echo = FALSE, fig.height = 4, fig.width = 8}
p.forms
```
{:.wide}
